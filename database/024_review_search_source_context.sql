-- 024_review_search_source_context.sql
-- Run after 023_review_items_labeled_view.sql.
-- Extends review search to include the sample source context shown in the UI.

create extension if not exists pg_trgm;

create index if not exists review_samples_context_search_trgm_idx
on public.review_samples
using gin ((lower(coalesce(current_source_text, ''))) gin_trgm_ops);

create or replace function public.count_review_items_filtered(p_request jsonb)
returns jsonb
language plpgsql
stable
security definer
    ...
    filtered as (
      ...
      where (v_entity_type is null or entity_type = v_entity_type)
        ...
        and (
          ...
          or lower(coalesce(current_source_text, '')) like '%' || v_search || '%'
        )
    )
...
grant execute on function public.count_review_items_filtered(jsonb) to authenticated;

create or replace function public.list_review_items_page_v2(p_request jsonb)
returns jsonb
language plpgsql
stable
security definer
    ...
    with page_rows as (
      ...
      where ri.dataset_id = v_dataset_id
        ...
        and (
          ...
          or lower(coalesce(rs.current_source_text, '')) like '%' || v_search || '%'
        )
        and (
          v_after is null
          or (
            ri.status::text < (v_after ->> 'status')
            or (
              ri.status::text = (v_after ->> 'status')
              and ri.entity_type > (v_after ->> 'entity_type')
            )
            or (
              ri.status::text = (v_after ->> 'status')
              and ri.entity_type = (v_after ->> 'entity_type')
              and ri.sample_key > (v_after ->> 'sample_key')
            )
            or (
              ri.status::text = (v_after ->> 'status')
              and ri.entity_type = (v_after ->> 'entity_type')
              ...
              and (
                case
                  when jsonb_typeof(v_after -> 'audit_record_id') = 'null' then (
                    ri.audit_record_id is null
                    and ri.id > (v_after ->> 'id')::uuid
                  )
                  else (
                    (
                      ri.audit_record_id is not null
                      and ri.audit_record_id > ((v_after ->> 'audit_record_id')::integer)
                    )
      ...
      select
        row_number() over (
          order by
            ...
            entity_type asc,
            sample_key asc,
            audit_record_id asc nulls last,
            id asc
        ) as page_row_number,
        page_rows.*
      from page_rows
      ...
      case
        when exists (select 1 from numbered where page_row_number > v_limit) then (
          select jsonb_build_object(
            ...
            'entity_type', entity_type,
            'sample_key', sample_key,
            'audit_record_id', to_jsonb(audit_record_id),
            'id', id::text
          )
          from last_row
        )
...
grant execute on function public.list_review_items_page_v2(jsonb) to authenticated;
